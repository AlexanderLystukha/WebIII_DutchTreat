...
namespace DutchTreat.Data.Repositories.Helpers
{
    public class RepositoryFactories
    {
        ...
        private IDictionary<Type, Func<ApplicationDbContext, object>> GetDucthFactories()
        {
            ...
            {
                {typeof(IDutchRepository<Product>), dbContext => new DutchProductRepository(dbContext, new Logger<DutchProductRepository>(_loggerFactory)) },
                {typeof(IDutchRepository<Order>), dbContext => new DutchOrderRepository(dbContext, new Logger<DutchOrderRepository>(_loggerFactory)) },
                {typeof(IDutchRepository<OrderItem>), dbContext => new DutchOrderItemRepository(dbContext, new Logger<DutchOrderItemRepository>(_loggerFactory)) }
        ...
        {
            return dbContext => new DutchGenericRepository<T>(dbContext, new Logger<DutchGenericRepository<T>>(_loggerFactory));
        }
    }
}
